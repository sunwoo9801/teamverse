...
public class ActivityLogController {
    ...
    public ActivityLogController(ActivityLogService activityLogService,
            ...
            ProjectRepository projectRepository) { // 생성자 주입
    ...
    @PostMapping("/task")
    public ResponseEntity<ActivityLogDTO> logTaskCreation(@RequestBody Map<String, Object> requestBody,
    ...
    // 게시글 수정 API
    @PutMapping("/post/{activityId}")
    @Transactional
    public ResponseEntity<ActivityLogDTO> updatePost(
        ...
        String title = requestBody.get("title") != null ? requestBody.get("title").toString() : "";
        String content = requestBody.get("content") != null ? requestBody.get("content").toString() : "";
        List<String> files = (List<String>) requestBody.getOrDefault("files", List.of());

        if (title.trim().isEmpty() || content.trim().isEmpty()) {
            return ResponseEntity.badRequest().build();
        }

        ActivityLogDTO updatedActivityLogDTO = activityLogService.updatePost(activityId, user, title, content, files);
        return ResponseEntity.ok(updatedActivityLogDTO);
    }

    // 게시글 삭제 API
    @DeleteMapping("/post/{activityId}")
    @Transactional
    public ResponseEntity<Void> deletePost(
        ...
        activityLogService.deletePost(activityId, user);
